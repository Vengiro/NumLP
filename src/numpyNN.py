...
def plot_decision_boundary(X, y, pred_fn, boundry_level=None, extra=False, f=None):
	"""
    ...
    :param boundry_level: Determines the number and positions of the contour lines / regions.
    :param extra: Whether to use an extra input feature.
    :param f: A function that computes the extra feature given (x, y).
    :return: None
    """
	...
	if extra:
		Z = pred_fn(np.c_[xx.ravel(), yy.ravel(), f(xx.ravel(), yy.ravel())])
	else:
